 ...
 *   • Энгийн нүүдэл — хүү (man) ЗӨВХӨН урагшаа диагоналиар нэг нүд,
 *     даам (king) хязгааргүй зайд диагоналиар (Chess bishop шиг "нисдэг").
 ...
 *   • Хүү ч ХОЙШОО идэж чадна (энгийн нүүдэл л урагшаа хязгаарлагдана).
 ...
 *   • Сүүлийн эгнээнд хүрвэл даам болно — хэрэв энэ нь идэлтийн ХЭЛХЭЭНИЙ
 *     дундуур тохиолдвол, ҮЛДСЭН хэсгийг ХААНААР үргэлжлүүлнэ (albeit
 *     даамын "нисдэг" идэлтээр).
...
export type DraughtsMove = {
   ...
   */
  landings: Square[];
  /** Энэ нүүдлийн төгсгөлд дүрс даам болсон эсэх. */
  promoted: boolean;
};
...
function captureSequencesFrom(
  ...
  for (const [dr, dc] of DIRS) {
    if (piece.king) {
      // Даам: хоосон нүдээр "нисээд", яг НЭГ дайсны дүрстэй тулгарна.
      ...
      // Хүү: яг зэргэлдээ нүдэн дэх дайсныг НЭГ алхмаар алгасна (АЛЬ Ч чиглэлээр).
      ...
      /*
       * ⚠ ДУНДАА ДАМКА БОЛОХГҮЙ. Урьд нь даамын эгнээнд хүрмэгц дүрсийг
       * даам болгож, үлдсэн хэлхээг ДАМКЫН дүрмээр (холын идэлт, хойш
       * идэлт) үргэлжлүүлж байв. Энэ нь хоёр алдаа үүсгэсэн:
       *
       *   1. ДҮРЭМ ЗӨРЧИГДӨНӨ. Олон улсын даамын дүрмээр хүү нь даамын
       *      эгнээг ДАМЖИН гарвал даам БОЛОХГҮЙ — зөвхөн тэр эгнээнд
       *      ЗОГСВОЛ даам болно.
       *   2. Хамгийн урт хэлхээ нь ЗӨВХӨН хууль бус даамын хүчээр
       *      үргэлжилдэг байсан тул «максималь идэлт» тэр хэлхээг
       *      сонгоно. Дүрс нь даамын эгнээнээс гарч буудаг тул
       *      `promoted: false` — сурагч «даам болсон дүрсээ идэлт хийсэн
       *      чинь даам болохоо больсон» гэж харна. Яг тэр гомдол.
       *
       * Одоо хэлхээ нь ХҮҮгийн дүрмээр үргэлжилнэ; даамын эгнээнд ЗОГССОН
...
export class Draughts {
   ...
   * дүрс, даам байсан эсэх) — түүнийг бүтнээр нь гаргавал дуудагч тал
  ...
  /** Одоогийн нүүх тал ХУУЛЬ ЁСНЫ нүүдэлгүй, эсвэл тэнцээний хязгаарт хүрсэн эсэх. */
  isGameOver(): boolean {
   ...
   *
   * ⚠ НҮҮДЭЛГҮЙ БОЛОХ нь ХОЖИГДОЛ БИШ, ТЭНЦЭЭ. Олон улсын даамын
   * дүрмээр нүүдэлгүй үлдсэн тал хожигддог ч ЭНЭ сайтын дүрмээр (эзний
   * шийдвэр) тэр нь тэнцээ. Хожил нь ЗӨВХӨН өрсөлдөгчийн БҮХ дүрсийг
   * идсэн үед тооцогдоно.
   *
   * ⚠ Тиймээс «боож хаах» нь хожлын арга БИШ: хичээл, бот, тоглолтын
   * дүгнэлт бүгд үүнийг ижил ойлгох ёстой (`lib/draughts/bot.ts`,
   * `play/draughts/*`, «Хожил» хичээл).
   */
  winner(): Color | null {
    if (this.noProgressCount >= NO_PROGRESS_DRAW_LIMIT) return null;

    const white = this.piecesOf("w").length;
    const black = this.piecesOf("b").length;
    if (black === 0 && white > 0) return "w";
