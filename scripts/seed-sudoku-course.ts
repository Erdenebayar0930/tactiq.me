...
/**
 * Хэмжээ тус бүрийн нэгж — хүндрэл гурав, тус бүр `TOTAL_PER_LEVEL` хичээл.
 *
 * ⚠ ХИЧЭЭЛ БҮР ЯГ НЭГ ХӨЛӨГТЭЙ, тохируулга биш. Хоёр шалтгаан:
 *
 *   1. Судоку нэг бүр нь өөрөө бүтэн бодлого. Хэд хэдэн хөлгийг нэг
 *      хичээлд хийхэд сурагчид «дуусахгүй» мэт мэдрэгддэг.
 *   2. XP нь ХИЧЭЭЛ дуусгахад олгогддог
 *      (`api/learn/lessons/[lessonId]/complete`) — тэр route нь лиг,
 *      хосын даалгавар, дараалал, курсын цаг, найзын урамшууллын «төв
 *      зангилаа» тул дасгал тутамд оноо олгохоор хуваах нь эдгээр
 *      бүгдийг давхар тоолох эрсдэлтэй. Хичээлд ганц судоку байвал
 *      «бөглөмөгц оноо» нь туршигдсан замаар ӨӨРӨӨ биелнэ.
 *
 * Зам хэт урт санагдвал `TOTAL_PER_LEVEL`-ийг багасга — хичээл дэх
 * хөлгийн тоог БИШ.
 *
 * XP-г хичээл тус бүрд админаас (`/admin/courses/sudoku`) засаж болно.
 */
function sizeUnit(
  size: SudokuSize,
  title: string,
  ...
  return {
    ...
    color,
    /*
     * Хүндрэл бүр → `TOTAL_PER_LEVEL` хичээл, тус бүр ганц хөлөгтэй.
     *
     * ⚠ ДУГААРЛАЛТЫГ PROMPT-ООС ХАССАН. Урьд нь «4×4 хөнгөн — 1/20» гэж
     * бичдэг байсан нь ТӨӨРӨГДҮҮЛНЭ: сурагч түүнийг «энэ хичээлд 20
     * дасгал байна» гэж уншина. Хичээлийн дугаар нь ГАРЧИГТ (`… 7`) аль
     * хэдийн байгаа тул prompt нь юу хийхийг л хэлэх ёстой.
     */
    lessons: LEVELS.flatMap(({ level, label }, levelIndex) =>
      Array.from({ length: TOTAL_PER_LEVEL }, (_, index) => ({
        title: `${size}×${size} ${label} ${index + 1}`,
        xp: xp[levelIndex],
        items: [
          {
            kind: "sudoku" as const,
            prompt: `1-ээс ${size} хүртэлх тоог бөглө.`,
            size,
            level,
            seed: seedFor(size, levelIndex, index),
            explanation: TIPS[size][level][index % TIPS[size][level].length],
          },
        ],
      }))
    ),
...
const UNITS: Unit[] = [
  ...
  /*
   * XP — [хөнгөн, дунд, хүнд]. Хичээл бүр ЯГ НЭГ хөлөгтэй тул эдгээр нь
   * «нэг судоку хэдэн оноотой» гэсэн үнэ: 4×4 хөнгөн нь хормын зуурын
   * ажил, 9×9 хүнд нь хэдэн минутын.
   */
  sizeUnit(4, "4×4 — хөнгөнөөс хүнд", "emerald", [5, 6, 8]),
  sizeUnit(6, "6×6 — хайрцаг дөрвөлжин биш", "amber", [8, 10, 12]),
  sizeUnit(9, "9×9 — жинхэнэ судоку", "violet", [10, 12, 15]),
];
