/**
 * Даамын «Нэг нүүдлийн цохилт» сэдвийг үүсгэх CLI.
 *
 * Ажиллуулах:
 ...
 *     ЦОРЫН ГАНЦ» гэсэн үг. Хоёр өөр цохилт байвал сурагч ЯГ ИЖИЛ САЙН
...
const UNIT_TITLE = "Нэг нүүдлийн цохилт";
const SIZE = 10;
const TOTAL = 20;
 ...
 *
 * ⚠ Цагаан бэр 0-р мөрөнд, хар бэр 9-р мөрөнд ТАВИГДАХГҮЙ: тэнд хүрсэн бэр
...
function toTask(board: Board): Task | null {
  ...
  // Хар тал ямар ч дүрсгүй үлдэх байрлал (цохилт нь тоглоомыг дуусгах)
  // сургалтын хувьд эргэлзээтэй: «цохилт ол» гэхээс илүү «сүүлчийн дүрсийг
...
async function main() {
  ...
  console.log(
    `✓ ${tasks.length} цохилтын даалгавар шалгалтад тэнцлээ ` +
  ...
  try {
    ...
    if (existingUnit) {
      ...
      // Сэдвийг ХАМГИЙН СҮҮЛД байрлуулна: цохилтын тактик нь дүрэм заасны
    ...
    for (let index = 0; index < tasks.length; index += 5) {
      ...
      for (const task of chunk) {
        await db.insert(exercises).values({
          ...
          prompt:
            task.captures === 1
              ? "Цагаанаар тоглож байна. Цохилтоо олж, хар дүрсийг ид."
